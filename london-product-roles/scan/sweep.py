#!/usr/bin/env python3
"""Stage 2 — Sweep & classify.

For every live board in data/ats_hits.json, pulls the current postings, keeps the
London-eligible ones, and classifies each as:

  PM          the title itself is a Product Management role
  PM-helpful  not a PM title, but the description explicitly asks for product
              judgment / sense / instinct (a candidate-facing signal, negations
              excluded)
...
# candidate-facing "product experience helps" phrasing (strict, avoids generic 'products')
VALUE = re.compile(
    r"product management experience|experience (?:in|with|as)[^.]{0,45}product manage|"
    r"\bas a product manager\b|product[- ]min(?:d|ded)|product mindset|product sense|"
    r"product instinct|product judg(?:e)?ment|product intuition|\bPM experience\b|"
    r"think like a product manager|product management background", re.I)
NEG = re.compile(r"\b(not|n't|without|no|don't|doesn't|isn't|aren't|rather than)\b")
...
def find_value(desc):
    """First non-negated VALUE match in the description, else None."""
    for m in VALUE.finditer(desc or ""):
        if NEG.search(desc[max(0, m.start() - 40):m.start()].lower()):
            continue
        return m
    return None

def classify(title, desc):
    if PMTITLE.search(title) and not NOTPM.search(title):
        return "PM", None
    m = find_value(desc)
    return ("PM-helpful", m) if m else (None, None)
...
roles = []
def add(company, source, title, tag, loctext, url, desc, comp="", posted=""):
    cat, m = classify(title, desc)
    if not cat:
        return
    ev = ""
    if cat == "PM-helpful":
        s = max(0, m.start() - 55); e = min(len(desc), m.end() + 55)
        ev = "…" + re.sub(r"\s+", " ", desc[s:e]).strip() + "…"
    roles.append(dict(company=company, source=source, title=title.strip(), category=cat,
                      tag=tag, location=re.sub(r"\s+", " ", loctext).strip()[:80],
                      url=url, comp=comp, evidence=ev, posted=posted))
...
def do_ashby(slug):
    ...
    for j in d.get("jobs", []):
        ...
        out.append((slug, "Ashby", j["title"], tag, txt, j.get("jobUrl", ""),
                    j.get("descriptionPlain", "") or "", comp, dt_iso(j.get("publishedAt"))))
    return out

def do_greenhouse(slug):
    out = []
    try:
        d = json.loads(curl(f"https://boards-api.greenhouse.io/v1/boards/{slug}/jobs?content=true"))
    except Exception:
        return out
    for j in d.get("jobs", []):
        ...
        out.append((slug, "Greenhouse", j["title"], tag, txt, j.get("absolute_url", ""),
                    strip_html(j.get("content", "")), "", posted))
    return out
...
def do_lever(slug):
    ...
    for j in d:
        ...
        out.append((slug, "Lever", j.get("text", ""), tag, txt, j.get("hostedUrl", ""),
                    j.get("descriptionPlain", "") or "", "", dt_ms(j.get("createdAt"))))
    return out
...
def do_workable(slug):
    ...
    for j in (d.get("jobs") or []):
        ...
        out.append((slug, "Workable", j.get("title", ""), tag, txt, j.get("url", ""),
                    strip_html(j.get("description", "")), "", posted))
    return out
...
def main():
    ...
    print(f"TOTAL {len(uniq)} roles | {dict(Counter(r['category'] for r in uniq))} "
          f"| dated {sum(1 for r in uniq if r['posted'])} -> {ROLES}")
